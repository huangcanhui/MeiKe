...
- (void)setSliderValue:(CGFloat)value animation:(BOOL)animation completion:(void (^)(BOOL))completion{
    ...
    CGPoint point = CGPointMake(value * kScreenWidth, 0);
...
- (void)setIsVertical:(BOOL)isVertical
{
    ...
    if (_isVertical == YES) {
        _thumbImageView.transform = CGAffineTransformMakeRotation(M_PI / 2);
    }
}
...
- (void)fillForeGroundViewWithPoint:(CGPoint)point{
    ...
    if (self.finishImage) {
        _thumbImageView.image = point.x  < (kScreenWidth - thunmbW/2) ? self.thumbImage : self.finishImage;
    }
    self.value = p.x  / kScreenWidth;
    
    
    _foregroundView.frame = CGRectMake(0, 0, point.x, kScreenHeight);
    ...
    }else if (_foregroundView.frame.size.width >= kScreenWidth) {
