...
@interface IndexView ()<AVCaptureMetadataOutputObjectsDelegate>
...
- (void)setOverView {
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, height / 2. - scanW / 2 + scanW + 5, width - 20, 50)];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    if (metadataObjects.count > 0) {
        ...
        if ([_delegate respondsToSelector:@selector(ybsView:ScanResult:)]) {
            [_delegate ybsView:self ScanResult:metadataObject.stringValue];
        }
    }
}
